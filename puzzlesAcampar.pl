...
% listasIguais([], []).
% listasIguais([C|R1], [C|R2]):-
%     listasIguais(R1, R2).

% mesmosEspacos(Tabuleiro, CopiaTabuleiro, Linhas, Colunas):-
%     todasCelulas(Tabuleiro, CelulasTabuleiro, var),
%     todasCelulas(CopiaTabuleiro, CelulasCopias, var),
%     length(CelulasCopias, N), N > 0,
%     length(CelulasTabuleiro, N2), N2 > 0,
%     listasIguais(CelulasTabuleiro, CelulasCopias),
%     tentativaErro(Tabuleiro, Linhas, Colunas).
% mesmosEspacos(Tabuleiro, _, Linhas, Colunas):-
%     resolve((Tabuleiro, Linhas, Colunas)).

% tendasVizinhancaAlargada(TodasTendas, Tenda):-
%     vizinhancaAlargada(Tenda, VizinhancaAlargada),
%     findall((X, Y),
%         (member((X, Y), VizinhancaAlargada), member((X, Y), TodasTendas)),
%         TendasVizinhancaAlargada
%     ), length(TendasVizinhancaAlargada, 0).
    
% tentativaErro(Tabuleiro, Linhas, Colunas):-
%     todasCelulas(Tabuleiro, CelulasVazias, var),
%     member((X, Y), CelulasVazias),
%     insereObjectoCelula(Tabuleiro, t, (X, Y)),
%     todasCelulas(Tabuleiro, Tendas, t),
%     maplist(tendasVizinhancaAlargada(Tendas), Tendas),
%     resolve((Tabuleiro, Linhas, Colunas)).

% resolve((Tabuleiro, Linhas, Colunas)):-
%     % Verificar se o puzzle esta resolvido
%     calculaObjectosTabuleiro(Tabuleiro, LinhasTendas, ColunasTendas, t),
%     listasIguais(LinhasTendas, Linhas), listasIguais(ColunasTendas, Colunas),
%     % Encontrar todas as celulas com arvores e com tendas
%     todasCelulas(Tabuleiro, LArv, a), todasCelulas(Tabuleiro, LTen, t),
%     maplist(tendasVizinhancaAlargada(LTen), LTen),
%     valida(LArv, LTen), !.
% resolve((Tabuleiro, Linhas, Colunas)):-
